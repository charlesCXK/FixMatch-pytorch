...
def main():
    ...
    def create_model(args):
        if args.arch == 'wideresnet':
            ...
            model = models.build_dualwideresnet(depth=args.model_depth,
...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            logits_u_left_w, logits_u_left_s = logits_left[batch_size:].chunk(2)
            logits_u_right_w, logits_u_right_s = logits_right[batch_size:].chunk(2)
            ...
            pseudo_label_left = torch.softmax(logits_u_left_w.detach()/args.T, dim=-1)
            max_probs_left, targets_u_left = torch.max(pseudo_label_left, dim=-1)
            pseudo_label_right = torch.softmax(logits_u_right_w.detach()/args.T, dim=-1)
            max_probs_right, targets_u_right = torch.max(pseudo_label_right, dim=-1)
            # mask = max_probs.ge(args.threshold).float()

            # Lu = (F.cross_entropy(logits_u_s, targets_u, reduction='none') * mask).mean()
            Lu = (F.cross_entropy(logits_u_left_s, targets_u_right, reduction='none')).mean() + (F.cross_entropy(logits_u_right_s, targets_u_left, reduction='none')).mean()
